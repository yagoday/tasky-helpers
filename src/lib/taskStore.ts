...
// Create a version of the store without persistence for environments where localStorage isn't available
const createTaskStore = (useLocalStorage = true) => {
  // Determine if we can access localStorage
  let canUseStorage = useLocalStorage;
  if (useLocalStorage) {
    try {
      ...
      console.warn("localStorage is not available - using in-memory store only");
      canUseStorage = false;
    }
  }
  ...
  const baseStore = {
    ...
    syncWithSupabase: async (userId?: string) => {
      ...
      try {
        // Skip table initialization and check directly
